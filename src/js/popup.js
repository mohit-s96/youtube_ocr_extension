...
// Send a message to the page's content script wAhich in turns calls a function to start the cropping / reading process
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    ...
    chrome.tabs.sendMessage(tabs[0].id, { todo: "sendPageInfo" }, (res) => {
      ...
      if (!result) {
        ...
        document
          .getElementById("btn")
          .addEventListener("click", startCropProcess);
      }
    });
  });
});
